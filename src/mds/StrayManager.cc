...
public:
  C_IO_PurgeStrayPurged(StrayManager *pq_, CDentry *d, bool oh, uint32_t ops) : 
    StrayManagerIOContext(pq_), dn(d), only_head(oh), ops_allowance(ops) { }
  void finish(int r) {
    assert(r == 0 || r == -ENOENT);
    pq->_purge_stray_purged(dn, ops_allowance, only_head);
  }
};
